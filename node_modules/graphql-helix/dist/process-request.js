...
const getExecutableOperation = (document, operationName) => {
    const operation = graphql_1.getOperationAST(document, operationName);
    if (!operation) {
...
const processRequest = async (options) => {
    ...
    const result = await (async () => {
        ...
        try {
            if (!util_1.isHttpMethod("GET", request.method) &&
            ...
            exports.validateDocument(schema, document, validate, validationRules);
            operation = getExecutableOperation(document, operationName);
            if (operation.operation === "mutation" &&
                util_1.isHttpMethod("GET", request.method)) {
                throw new errors_1.HttpError(405, "Can only perform a mutation operation from a POST request.", { headers: [{ name: "Allow", value: "POST" }] });
            }
            let variableValues;
            try {
                if (variables) {
            ...
            try {
                ...
                if (operation.operation === "subscription") {
                    ...
                    if (util_1.isAsyncIterable(result)) {
                        return {
                            type: "PUSH",
                            ...
                            unsubscribe: () => {
                                util_1.stopAsyncIteration(result);
                            },
                        };
                    }
                    else {
